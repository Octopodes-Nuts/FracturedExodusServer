...
func (api *PlayerAPI) handleCharacters(response http.ResponseWriter, request *http.Request) {
	...
	rows, err := server.SubmitQuery(context.Background(), db.DB, query, charactersRequest.SessionToken, time.Now().UTC())
	if err != nil {
		response.WriteHeader(http.StatusInternalServerError)
		_ = json.NewEncoder(response).Encode(map[string]any{
			"status":  "error",
...
func (api *PlayerAPI) handleNewCharacter(response http.ResponseWriter, request *http.Request) {
	...
	query := "SELECT player_id FROM session_tokens WHERE session_token = $1 AND expiration > $2"
	rows, err := server.SubmitQuery(context.Background(), db.DB, query, newCharacterRequest.SessionToken, time.Now().UTC())
	if err != nil {
	...
	if _, err := server.SubmitExec(context.Background(), db.DB, query, player_id, id, newCharacterRequest.Name, newCharacterRequest.SkinKey, newCharacterRequest.Weapon1, newCharacterRequest.Weapon2, newCharacterRequest.Weapon3, newCharacterRequest.Equipment1, newCharacterRequest.Equipment2, newCharacterRequest.ClassType, newCharacterRequest.Faction); err != nil {
...
func (api *PlayerAPI) handleDeleteCharacter(response http.ResponseWriter, request *http.Request) {
	...
	result, err := server.SubmitExec(context.Background(), db.DB, query, deleteRequest.CharacterID, playerID)
	if err != nil {
		response.WriteHeader(http.StatusInternalServerError)
		_ = json.NewEncoder(response).Encode(map[string]any{
			"status":  "error",
